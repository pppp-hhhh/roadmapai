...
#[tauri::command]
pub async fn get_api_key(state: State<'_, AppState>, provider: String) -> Result<String, String> {
    let db = state.db.lock().await;
...
#[tauri::command]
pub async fn set_ai_provider(state: State<'_, AppState>, provider: String) -> Result<(), String> {
    let db = state.db.lock().await;
...
pub async fn test_connection(
    ...
    let api_key = {
        let db = state.db.lock().await;
        db.get_api_key(&provider).await?.unwrap_or_default()
    };
    ...
    let ai_provider: Box<dyn AiProvider> = match config.provider_type.as_str() {
        "anthropic" => Box::new(crate::services::ClaudeProvider::new()),
        _ => Box::new(crate::services::CustomProvider::new(
